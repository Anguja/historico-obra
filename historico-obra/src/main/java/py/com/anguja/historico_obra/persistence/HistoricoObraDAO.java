...
public class HistoricoObraDAO {
	...
	public void crearHistoricoObra(HistObra historico) {
		Cliente cliente = em.getReference(Cliente.class, historico.getCliente().getIdCliente());
		historico.setCliente(cliente);
		em.persist(historico);
	}

	// nombre, fecha_fin, fecha_inicio, tipo de obra, obra tarea, monto de
	// contrato, ubicacion, consorcio, descripcion de la obra
	...
	public void actualizarHistoricoObra(HistObra historico) {
		HistObra histObra = em.getReference(HistObra.class, historico.getIdHistObra());

		histObra.setActivo(historico.getActivo());
